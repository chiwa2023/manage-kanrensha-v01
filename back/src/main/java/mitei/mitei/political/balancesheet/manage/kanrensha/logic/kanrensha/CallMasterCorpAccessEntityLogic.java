 ...
 */
@Component
public class CallMasterCorpAccessEntityLogic {
     ...
     * @return 企業団体連絡先情報
     */
    public MasterCorporationAccessEntity practice(final String corpKanrenshaCode) {
        // データベースよりcorpKanrenshaCodeが一致し、かつテーブルIdの降順で、リストを取得
        List<MasterCorporationAccessEntity> entities = masterCorporationAccessRepository
                .findByCorpKanrenshaCodeOrderByMasterCorporationAccessIdDesc(corpKanrenshaCode);

        // 取得したリストが空の場合、新たにEntityのインスタンスを作成し、corpKanrenshaCodeだけを設定して返却
        if (entities.isEmpty()) {
            MasterCorporationAccessEntity newEntity = new MasterCorporationAccessEntity();
            newEntity.setCorpKanrenshaCode(corpKanrenshaCode);
            return newEntity;
        }

        // 取得したリストをisLatestがtrueでフィルタ
        List<MasterCorporationAccessEntity> latestEntities = entities.stream().filter(MasterCorporationAccessEntity::getIsLatest)
        ...
        if (nomalCnt < latestEntities.size()) {
            throw new DataRetrievalFailureException(
                    "Multiple latest records found for corpKanrenshaCode: " + corpKanrenshaCode);
        }
