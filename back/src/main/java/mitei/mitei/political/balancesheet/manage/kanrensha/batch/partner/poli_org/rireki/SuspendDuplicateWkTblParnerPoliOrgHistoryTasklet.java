 ...
 */
@Component
public class SuspendDuplicateWkTblParnerPoliOrgHistoryTasklet implements Tasklet,StepExecutionListener {
    ...
    public RepeatStatus execute(final StepContribution contribution, final ChunkContext chunkContext) throws Exception {
        ...
        List<PartnerPoliOrgUniquekeyDto> listKeyGroup = wkTblPartnerPoliOrgHistoryRepository.findDuplicateUniqueKey(userCode);

        for (PartnerPoliOrgUniquekeyDto uniqueDto : listKeyGroup) {
            List<WkTblPartnerPoliOrgHistoryEntity> list = wkTblPartnerPoliOrgHistoryRepository
                    .findByPartnerNameAndAllAddressAndPoliOrgDelegateAndPoliOrgKanrenshaCodeAndInsertUserCodeOrderByWkPartnerPoliOrgHistoryIdAsc(
                            uniqueDto.getPartnerName(), uniqueDto.getAllAddress(), uniqueDto.getPoliOrgDelegate(),
                            uniqueDto.getPoliOrgKanrenshaCode(), userCode);
            list.remove(0); // 1行だけは処理実行行として残す
            for (WkTblPartnerPoliOrgHistoryEntity entity : list) {
                setTableDataHistoryUtil.practiceDelete(userDto, entity); // 削除
                entity.setIsFinish(true);
                entity.setJudgeReason("アップロードファイル内で重複しているデータです");
            }
            wkTblPartnerPoliOrgHistoryRepository.saveAllAndFlush(list);
        }
