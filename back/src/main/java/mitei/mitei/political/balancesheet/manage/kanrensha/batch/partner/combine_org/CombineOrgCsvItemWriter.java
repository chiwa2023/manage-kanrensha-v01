 ...
 */
@Component
public class CombineOrgCsvItemWriter extends JpaItemWriter<WkTblPartnerCombineOrgEntity> {
    ...
    public void write(final Chunk<? extends WkTblPartnerCombineOrgEntity> items) {
        ...
        Optional<WkTblPartnerCombineOrgEntity> optional = wkTblPartnerCombineOrgRepository
                .findFirstByOrderByWkTblPartnerCombineOrgCodeDesc();
        if (!optional.isEmpty()) {
            code = optional.get().getWkTblPartnerCombineOrgCode();
        }

        for (WkTblPartnerCombineOrgEntity entity : items) {
            code++;
            setTableDataHistoryUtil.practiceInsert(userDto, entity);
            entity.setWkTblPartnerCombineOrgCode(code);
        }
