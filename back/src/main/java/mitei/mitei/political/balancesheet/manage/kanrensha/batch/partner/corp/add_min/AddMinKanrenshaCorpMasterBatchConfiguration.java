 ...
 */
@Configuration
public class AddMinKanrenshaCorpMasterBatchConfiguration {

    /** 機能名 */
    private static final String FUNCTION_NAME = "addMinKanrenshaCorpMasterBatch";
    ...
    @Autowired
    private PartnerCorpAddMiniCsvItemWriter partnerCorpAddMiniCsvItemWriter;

    /** ファイル内重複データ処理停止Tasklet */
    @Autowired
    private SuspendDuplicateWkTblParnerCorpAddMinTasklet suspendDuplicateWkTblParnerCorpAddMinTasklet;
    ...
    protected Step getStepErase(final JobRepository jobRepository,
        ...
        return new StepBuilder(STEP_ERASE, jobRepository).tasklet(eraseWkTblParnerCorpAddMinTasklet, transactionManager)
                .build();
    }
    ...
    protected Step getStepCsv(final JobRepository jobRepository, final PlatformTransactionManager transactionManager) {
        ...
        return new StepBuilder(STEP_CSV, jobRepository)
                .<PartnerCorpAddMiniDto, WkTblPartnerCorpAddMinEntity>chunk(CHUNK_SIZE, transactionManager)
                .reader(partnerCorpAddMiniCsvItemReader).processor(partnerCorpAddMiniCsvProcessor)
                .writer(partnerCorpAddMiniCsvItemWriter).build();
    }
    ...
    protected Step getStepSuspend(final JobRepository jobRepository,
        ...
        return new StepBuilder(STEP_SUSPEND, jobRepository)
                .tasklet(suspendDuplicateWkTblParnerCorpAddMinTasklet, transactionManager).build();
    }
