 ...
 */
@Component
public class UpdateApprovalKanrenshaCorpAddressLogic {
    ...
    public Integer practice(final MasterKanrenshaAddressBaseEntity entity, final UserPersonLeastDto userDto) {
        ...
        MasterCorporationAddressEntity entityLoad = masterCorporationAddressRepository
        ...
        if (Objects.equals(entity.getPostal1(), entityLoad.getPostal1())
            ...
            masterCorporationAddressRepository.save(entityLoad);

        }

        // TODO マスタ本体も修正

        if (0 == masterCorporationAddressRepository.save(this.cloneNewEntity(entity, entityLoad, userDto))
                .getMasterCorporationAddressId()) {
            return 0;
        } else {
    ...
    private MasterCorporationAddressEntity cloneNewEntity(final MasterKanrenshaAddressBaseEntity entity,
            final MasterCorporationAddressEntity entityLoad, final UserPersonLeastDto userDto) {

        MasterCorporationAddressEntity entitySaved = new MasterCorporationAddressEntity();
        BeanUtils.copyProperties(entity, entitySaved);

        entitySaved.setCorpKanrenshaCode(entityLoad.getCorpKanrenshaCode());

        setTableDataHistoryUtil.practiceInsert(userDto, entitySaved);
        entitySaved.setMasterCorporationAddressId(0); // auto increment
        return entitySaved;
    }
