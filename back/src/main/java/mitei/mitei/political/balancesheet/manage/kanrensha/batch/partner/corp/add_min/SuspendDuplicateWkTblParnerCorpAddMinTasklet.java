 ...
 */
@Component
public class SuspendDuplicateWkTblParnerCorpAddMinTasklet implements Tasklet, StepExecutionListener {
    ...
    public RepeatStatus execute(final StepContribution contribution, final ChunkContext chunkContext) throws Exception {
        ...
        List<PartnerCorpMasterUniquekeyDto> listKeyGroup = wkTblPartnerCorpAddMinRepository
                .findDuplicateUniqueKey(userCode);

        for (PartnerCorpMasterUniquekeyDto uniqueDto : listKeyGroup) {
            List<WkTblPartnerCorpAddMinEntity> list = wkTblPartnerCorpAddMinRepository
                    .findByPartnerNameAndAllAddressAndCorpDelegateAndInsertUserCodeOrderByWkTblPartnerCorpAddMinIdAsc(
                            uniqueDto.getPartnerName(), uniqueDto.getAllAddress(), uniqueDto.getCorpDelegate(),
                            userCode);
            list.remove(0); // 1行だけは処理実行行として残す
            for (WkTblPartnerCorpAddMinEntity entity : list) {
                setTableDataHistoryUtil.practiceDelete(userDto, entity); // 削除
                entity.setIsFinish(true);
                entity.setJudgeReason("アップロードファイル内で重複しているデータです");
            }
            wkTblPartnerCorpAddMinRepository.saveAllAndFlush(list);
        }
