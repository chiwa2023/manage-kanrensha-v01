...
public interface PartnerCorpHistory01Repository extends JpaRepository<PartnerCorpHistory01Entity, Integer> {
     ...
     * @return 検索結果
     */
    Page<PartnerCorpHistory01Entity> findByInsertTimestampLessThanAndIsLatest(LocalDateTime dateTime, boolean isLatest,
            Pageable pageable);
     ...
     * @return 検索結果
     */
    Page<PartnerCorpHistory01Entity> findByInsertTimestampGreaterThanEqualAndInsertTimestampLessThanAndIsLatest(
    ...
    @Query(value = "SELECT  1 AS kanrensha_kbn ,partner_name ,all_address "
            ...
            + "   FROM partner_person_history_01 "
            ...
            + "   FROM partner_poli_org_history_01 "
            + "       WHERE partner_name = ?1 AND all_address = ?2"
            + "           AND is_latest = 1"
            , nativeQuery = true)
