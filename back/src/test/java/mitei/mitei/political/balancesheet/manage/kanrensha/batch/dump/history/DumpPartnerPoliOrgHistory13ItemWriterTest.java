...
/**
 * DumpPartnerPoliOrgHistory13ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory13ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org13.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory13ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory13Entity entity00 = new PartnerPoliOrgHistory13Entity();
        ...
        List<PartnerPoliOrgHistory13Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory13Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory13ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory13ItemWriter.write(items);
