...
/**
 * DumpPartnerPoliOrgHistory30ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory30ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org30.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory30ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory30Entity entity00 = new PartnerPoliOrgHistory30Entity();
        ...
        List<PartnerPoliOrgHistory30Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory30Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory30ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory30ItemWriter.write(items);
