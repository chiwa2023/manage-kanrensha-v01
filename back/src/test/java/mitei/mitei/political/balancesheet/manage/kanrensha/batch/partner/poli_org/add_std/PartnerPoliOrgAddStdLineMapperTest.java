...
/**
 * PartnerPoliOrgAddStdLineMapper単体テスト
 */
@SpringJUnitConfig
@AutoConfigureMockMvc
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT)
@DirtiesContext(classMode = ClassMode.BEFORE_CLASS)
class PartnerPoliOrgAddStdLineMapperTest {

    /** テスト対象 */
    @Autowired
    private PartnerPoliOrgAddStdLineMapper partnerPoliOrgAddStdLineMapper;
    ...
    void test() throws Exception { // NOPMD
        ...
        // 団体区分カラム位置
        final String dantaiKbn = "401";
        builder.append(quote).append(dantaiKbn).append(quote).append(comma);
        ...
        // 関連者団体名称かな
        final String orgKana = "ちゃらんぽらんせいじだんたい";
        builder.append(quote).append(orgKana).append(quote).append(comma);

        // 団体代表者関連者コード
        final String delegateCode = "222-3333";
        builder.append(quote).append(delegateCode).append(quote).append(comma);

        // 会計責任者関連者個人コード
        final String accCode = "333-444";
        builder.append(quote).append(accCode).append(quote).append(comma);

        // 会計責任者関連者個人氏名
        final String accName = "会計責任者　花子";
        builder.append(quote).append(accName).append(quote).append(comma);
        ...
        PartnerPoliOrgAddStdDto dto = partnerPoliOrgAddStdLineMapper.mapLine(builder.toString(), 0);

        assertEquals(name, dto.getPartnerName());
        assertEquals(address, dto.getAllAddress());
        assertEquals(delegate, dto.getPoliOrgDelegate());
        assertEquals(dantaiKbn, dto.getDantaiKbn());
        ...
        assertEquals(orgKana, dto.getOrgNameKana());
        assertEquals(delegateCode, dto.getOrgDelegateCode());
        assertEquals(accCode, dto.getAccountMgrCode());
        assertEquals(accName, dto.getAccountMgrName());
    }
