...
/**
 * DumpPartnerCorpHistory16ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory16ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_16.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory16ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory16Entity entity00 = new PartnerCorpHistory16Entity();
        ...
        List<PartnerCorpHistory16Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory16Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory16ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory16ItemWriter.write(items);
