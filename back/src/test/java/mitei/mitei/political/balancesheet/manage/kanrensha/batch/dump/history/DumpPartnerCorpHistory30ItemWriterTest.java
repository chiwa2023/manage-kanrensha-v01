...
/**
 * DumpPartnerCorpHistory30ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory30ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_30.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory30ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory30Entity entity00 = new PartnerCorpHistory30Entity();
        ...
        List<PartnerCorpHistory30Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory30Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory30ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory30ItemWriter.write(items);
