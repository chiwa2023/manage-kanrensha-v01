...
/**
 * DumpPartnerPersonHistory41ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory41ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_41.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory41ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory41Entity entity00 = new PartnerPersonHistory41Entity();
        ...
        List<PartnerPersonHistory41Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory41Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory41ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory41ItemWriter.write(items);
