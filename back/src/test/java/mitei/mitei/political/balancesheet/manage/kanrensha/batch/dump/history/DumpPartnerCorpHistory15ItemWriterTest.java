...
/**
 * DumpPartnerCorpHistory15ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory15ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_15.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory15ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory15Entity entity00 = new PartnerCorpHistory15Entity();
        ...
        List<PartnerCorpHistory15Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory15Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory15ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory15ItemWriter.write(items);
