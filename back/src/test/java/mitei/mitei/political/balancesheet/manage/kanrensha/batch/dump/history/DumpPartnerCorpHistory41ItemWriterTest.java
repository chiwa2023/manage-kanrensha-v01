...
/**
 * DumpPartnerCorpHistory41ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory41ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_41.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory41ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory41Entity entity00 = new PartnerCorpHistory41Entity();
        ...
        List<PartnerCorpHistory41Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory41Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory41ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory41ItemWriter.write(items);
