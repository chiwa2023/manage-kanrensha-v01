...
/**
 * DumpPartnerPoliOrgHistory06ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory06ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org06.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory06ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory06Entity entity00 = new PartnerPoliOrgHistory06Entity();
        ...
        List<PartnerPoliOrgHistory06Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory06Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory06ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory06ItemWriter.write(items);
