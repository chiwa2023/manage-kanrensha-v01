...
/**
 * DumpPartnerPoliOrgHistory08ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory08ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org08.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory08ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory08Entity entity00 = new PartnerPoliOrgHistory08Entity();
        ...
        List<PartnerPoliOrgHistory08Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory08Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory08ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory08ItemWriter.write(items);
