...
/**
 * DumpPartnerPersonHistory21ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory21ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_21.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory21ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory21Entity entity00 = new PartnerPersonHistory21Entity();
        ...
        List<PartnerPersonHistory21Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory21Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory21ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory21ItemWriter.write(items);
