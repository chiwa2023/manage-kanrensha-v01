...
/**
 * DumpPartnerCorpHistory36ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory36ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_36.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory36ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory36Entity entity00 = new PartnerCorpHistory36Entity();
        ...
        List<PartnerCorpHistory36Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory36Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory36ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory36ItemWriter.write(items);
