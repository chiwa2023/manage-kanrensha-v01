...
/**
 * DumpPartnerPersonHistory40ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory40ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_40.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory40ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory40Entity entity00 = new PartnerPersonHistory40Entity();
        ...
        List<PartnerPersonHistory40Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory40Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory40ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory40ItemWriter.write(items);
