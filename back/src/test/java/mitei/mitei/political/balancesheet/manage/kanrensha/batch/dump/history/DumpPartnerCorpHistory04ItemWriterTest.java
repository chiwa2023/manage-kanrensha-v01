...
/**
 * DumpPartnerCorpHistory04ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory04ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_04.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory04ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory04Entity entity00 = new PartnerCorpHistory04Entity();
        ...
        List<PartnerCorpHistory04Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory04Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory04ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory04ItemWriter.write(items);
