...
/**
 * RepairHoujiNoBatchConfiguration単体テスト
 */
@SpringJUnitConfig
...
class RepairHoujiNoBatchConfigurationTest {
    ...
    /** 起動をするJob */
    @Qualifier(RepairHoujiNoBatchConfiguration.JOB_NAME)
    @Autowired
    private Job repairHoujiNo;

    @Test
    @Tag("TableTruncate")
    void testJob() {
        assertEquals(RepairHoujiNoBatchConfiguration.JOB_NAME, repairHoujiNo.getName(), "Job名が一致");
    }

    @Test
    @Tag("TableTruncate")
    @Sql({ "remove_houjin_no_history.sql", "houjin_no_latest.sql" })
    void testExecute() throws Exception {

        // TODO 複数ファイル=複数県実施できる環境になった段階で実施する
        jobLauncherTestUtils.setJob(repairHoujiNo);
        ...
        JobParameters jobParameters = new JobParametersBuilder(
                repairHoujiNo.getJobParametersIncrementer().getNext(new JobParameters())) // NOPMD
