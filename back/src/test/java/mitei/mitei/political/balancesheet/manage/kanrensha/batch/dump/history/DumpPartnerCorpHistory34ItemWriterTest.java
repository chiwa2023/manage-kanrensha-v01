...
/**
 * DumpPartnerCorpHistory34ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory34ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_34.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory34ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory34Entity entity00 = new PartnerCorpHistory34Entity();
        ...
        List<PartnerCorpHistory34Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory34Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory34ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory34ItemWriter.write(items);
