...
/**
 * DumpPartnerPersonHistory25ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory25ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_25.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory25ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory25Entity entity00 = new PartnerPersonHistory25Entity();
        ...
        List<PartnerPersonHistory25Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory25Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory25ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory25ItemWriter.write(items);
