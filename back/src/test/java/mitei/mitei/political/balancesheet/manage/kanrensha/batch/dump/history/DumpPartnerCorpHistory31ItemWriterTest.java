...
/**
 * DumpPartnerCorpHistory31ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory31ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_31.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory31ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory31Entity entity00 = new PartnerCorpHistory31Entity();
        ...
        List<PartnerCorpHistory31Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory31Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory31ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory31ItemWriter.write(items);
