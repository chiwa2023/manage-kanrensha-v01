...
/**
 * DumpPartnerPersonHistory18ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory18ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_18.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory18ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory18Entity entity00 = new PartnerPersonHistory18Entity();
        ...
        List<PartnerPersonHistory18Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory18Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory18ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory18ItemWriter.write(items);
