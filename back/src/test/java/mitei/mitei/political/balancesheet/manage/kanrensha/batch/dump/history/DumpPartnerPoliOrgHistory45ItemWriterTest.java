...
/**
 * DumpPartnerPoliOrgHistory45ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory45ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org45.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory45ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory45Entity entity00 = new PartnerPoliOrgHistory45Entity();
        ...
        List<PartnerPoliOrgHistory45Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory45Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory45ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory45ItemWriter.write(items);
