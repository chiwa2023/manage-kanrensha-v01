...
/**
 * AddMinKanrenshaCorpMasterBatchConfiguration単体テスト
 */
@SpringJUnitConfig
...
class AddMinKanrenshaCorpMasterBatchConfigurationTest {
    ...
    /** 起動をするJob */
    @Qualifier(AddMinKanrenshaCorpMasterBatchConfiguration.JOB_NAME)
    @Autowired
    private Job addMinKanrenshaCorpMaster;
    ...
    @Tag("TableTruncate")
    void testJob() {
        assertEquals(AddMinKanrenshaCorpMasterBatchConfiguration.JOB_NAME, addMinKanrenshaCorpMaster.getName(),
                "Job名が一致");
    }
    ...
    void testExecute() throws Exception {
        ...
        jobLauncherTestUtils.setJob(addMinKanrenshaCorpMaster);

        Path path = Paths.get(GetCurrentResourcePath.getBackTestResourcePath(), "batch/partner_corp",
                "関連者企業団体最小登録.csv");

        JobParameters jobParameters = new JobParametersBuilder(
                addMinKanrenshaCorpMaster.getJobParametersIncrementer().getNext(new JobParameters())) // NOPMD
