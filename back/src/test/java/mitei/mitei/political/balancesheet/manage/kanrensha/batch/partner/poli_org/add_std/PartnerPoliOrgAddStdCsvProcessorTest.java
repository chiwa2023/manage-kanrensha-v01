...
/**
 * PartnerPoliOrgAddStdCsvProcessor単体テスト
 */
@SpringJUnitConfig
@AutoConfigureMockMvc
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT)
@DirtiesContext(classMode = ClassMode.BEFORE_CLASS)
class PartnerPoliOrgAddStdCsvProcessorTest {
    ...
    @Tag("TableTruncate")
    @Transactional
    @Sql({ "sample_master_poli_org.sql", "sample_partner_poli_org_history.sql" })
    void test() throws Exception {

        // 未入力カラムがあると追加作業をしません
        WkTblMasterPoliOrgEntity wktblEntity00 = partnerPoliOrgAddStdCsvProcessor
                .process(new PartnerPoliOrgAddStdDto());
        assertEquals(false, wktblEntity00.getIsAffected());

        // 未入力エラーメッセージ
        StringBuilder builder0 = new StringBuilder();
        builder0.append("名称が入力されていません;") // NOPMD
                .append("住所が入力されていません;").append("代表者が入力されていません;").append("団体区分が入力されていません;")
        ...
        PartnerPoliOrgAddStdDto dto01 = this.createHissuInputDto();
        ...
        WkTblMasterPoliOrgEntity wktblEntity01 = partnerPoliOrgAddStdCsvProcessor.process(dto01);
        ...
        PartnerPoliOrgAddStdDto dto02 = this.createHissuInputDto();
        dto02.setPartnerName("いいかげん政治団体");
        dto02.setAllAddress("北海道架空市湖畔町");
        dto02.setPoliOrgDelegate("代表者　次郎");
        WkTblMasterPoliOrgEntity wktblEntity02 = partnerPoliOrgAddStdCsvProcessor.process(dto02);
        ...
        PartnerPoliOrgAddStdDto dto03 = this.createHissuInputDto();
        dto03.setPartnerName("陰謀論政治団体");
        dto03.setAllAddress("山形県架空市実在町");
        dto03.setPoliOrgDelegate("陰謀論　三郎");

        WkTblMasterPoliOrgEntity wktblEntity03 = partnerPoliOrgAddStdCsvProcessor.process(dto03);
        assertEquals(false, wktblEntity03.getIsAffected());
        assertEquals("同名の団体があります。確認調査の上、必要に応じて追加してください;", wktblEntity03.getJudgeReason());

        // 全くの新規であれば追加作業します
        PartnerPoliOrgAddStdDto dto04 = this.createHissuInputDto();
        dto04.setPartnerName("ちゃらんぽらん政治団体");
        dto04.setAllAddress("宮崎県架空市実在町");
        dto03.setPoliOrgDelegate("代表者　太郎");

        WkTblMasterPoliOrgEntity wktblEntity04 = partnerPoliOrgAddStdCsvProcessor.process(dto04);
        assertEquals(true, wktblEntity04.getIsAffected());

    }

    private PartnerPoliOrgAddStdDto createHissuInputDto() {
        PartnerPoliOrgAddStdDto dto = new PartnerPoliOrgAddStdDto();

        dto.setPartnerName("のんびり政治団体");
        dto.setAllAddress("北海道架空市湖畔町");
        dto.setPoliOrgDelegate("政治団体　花子");
        dto.setDantaiKbn("05");
