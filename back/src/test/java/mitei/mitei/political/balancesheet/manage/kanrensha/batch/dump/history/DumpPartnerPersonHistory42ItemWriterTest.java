...
/**
 * DumpPartnerPersonHistory42ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory42ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_42.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory42ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory42Entity entity00 = new PartnerPersonHistory42Entity();
        ...
        List<PartnerPersonHistory42Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory42Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory42ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory42ItemWriter.write(items);
