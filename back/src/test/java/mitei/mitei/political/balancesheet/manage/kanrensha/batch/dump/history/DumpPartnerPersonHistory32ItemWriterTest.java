...
/**
 * DumpPartnerPersonHistory32ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory32ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_32.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory32ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory32Entity entity00 = new PartnerPersonHistory32Entity();
        ...
        List<PartnerPersonHistory32Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory32Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory32ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory32ItemWriter.write(items);
