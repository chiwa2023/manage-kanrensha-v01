...
/**
 * DumpPartnerPoliOrgHistory15ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory15ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org15.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory15ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory15Entity entity00 = new PartnerPoliOrgHistory15Entity();
        ...
        List<PartnerPoliOrgHistory15Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory15Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory15ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory15ItemWriter.write(items);
