...
/**
 * DumpPartnerPoliOrgHistory07ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory07ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org07.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory07ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory07Entity entity00 = new PartnerPoliOrgHistory07Entity();
        ...
        List<PartnerPoliOrgHistory07Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory07Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory07ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory07ItemWriter.write(items);
