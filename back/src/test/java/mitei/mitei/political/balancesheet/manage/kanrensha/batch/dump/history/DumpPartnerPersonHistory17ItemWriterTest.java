...
/**
 * DumpPartnerPersonHistory17ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory17ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_17.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory17ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory17Entity entity00 = new PartnerPersonHistory17Entity();
        ...
        List<PartnerPersonHistory17Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory17Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory17ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory17ItemWriter.write(items);
