...
/**
 * DumpPartnerPoliOrgHistory19ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory19ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org19.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory19ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory19Entity entity00 = new PartnerPoliOrgHistory19Entity();
        ...
        List<PartnerPoliOrgHistory19Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory19Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory19ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory19ItemWriter.write(items);
