...
/**
 * DumpPartnerCorpHistory26ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory26ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_26.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory26ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory26Entity entity00 = new PartnerCorpHistory26Entity();
        ...
        List<PartnerCorpHistory26Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory26Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory26ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory26ItemWriter.write(items);
