...
/**
 * DumpPartnerCorpHistory13ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory13ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_13.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory13ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory13Entity entity00 = new PartnerCorpHistory13Entity();
        ...
        List<PartnerCorpHistory13Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory13Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory13ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory13ItemWriter.write(items);
