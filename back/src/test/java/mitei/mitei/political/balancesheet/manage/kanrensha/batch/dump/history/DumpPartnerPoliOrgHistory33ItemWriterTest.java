...
/**
 * DumpPartnerPoliOrgHistory33ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory33ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org33.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory33ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory33Entity entity00 = new PartnerPoliOrgHistory33Entity();
        ...
        List<PartnerPoliOrgHistory33Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory33Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory33ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory33ItemWriter.write(items);
