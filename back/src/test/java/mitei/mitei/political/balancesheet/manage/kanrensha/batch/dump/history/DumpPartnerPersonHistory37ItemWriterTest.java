...
/**
 * DumpPartnerPersonHistory37ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory37ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_37.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory37ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory37Entity entity00 = new PartnerPersonHistory37Entity();
        ...
        List<PartnerPersonHistory37Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory37Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory37ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory37ItemWriter.write(items);
