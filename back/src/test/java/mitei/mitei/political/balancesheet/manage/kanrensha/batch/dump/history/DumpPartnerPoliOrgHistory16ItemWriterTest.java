...
/**
 * DumpPartnerPoliOrgHistory16ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory16ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org16.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory16ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory16Entity entity00 = new PartnerPoliOrgHistory16Entity();
        ...
        List<PartnerPoliOrgHistory16Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory16Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory16ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory16ItemWriter.write(items);
