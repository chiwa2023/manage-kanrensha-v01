...
/**
 * DumpPartnerPersonHistory36ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory36ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_36.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory36ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory36Entity entity00 = new PartnerPersonHistory36Entity();
        ...
        List<PartnerPersonHistory36Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory36Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory36ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory36ItemWriter.write(items);
