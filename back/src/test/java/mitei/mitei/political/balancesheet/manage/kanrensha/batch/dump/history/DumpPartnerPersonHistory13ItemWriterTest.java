...
/**
 * DumpPartnerPersonHistory13ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory13ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_13.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory13ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory13Entity entity00 = new PartnerPersonHistory13Entity();
        ...
        List<PartnerPersonHistory13Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory13Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory13ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory13ItemWriter.write(items);
