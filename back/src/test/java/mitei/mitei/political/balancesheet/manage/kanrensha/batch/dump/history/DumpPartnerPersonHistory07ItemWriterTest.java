...
/**
 * DumpPartnerPersonHistory07ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory07ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_07.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory07ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory07Entity entity00 = new PartnerPersonHistory07Entity();
        ...
        List<PartnerPersonHistory07Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory07Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory07ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory07ItemWriter.write(items);
