...
/**
 * DumpPartnerPoliOrgHistory05ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory05ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org05.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory05ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory05Entity entity00 = new PartnerPoliOrgHistory05Entity();
        ...
        List<PartnerPoliOrgHistory05Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory05Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory05ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory05ItemWriter.write(items);
