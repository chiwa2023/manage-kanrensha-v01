...
/**
 * DumpPartnerCorpHistory43ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory43ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_43.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory43ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory43Entity entity00 = new PartnerCorpHistory43Entity();
        ...
        List<PartnerCorpHistory43Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory43Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory43ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory43ItemWriter.write(items);
