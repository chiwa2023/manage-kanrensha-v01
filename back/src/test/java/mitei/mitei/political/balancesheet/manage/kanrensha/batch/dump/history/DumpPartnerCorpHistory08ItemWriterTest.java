...
/**
 * DumpPartnerCorpHistory08ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory08ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_08.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory08ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory08Entity entity00 = new PartnerCorpHistory08Entity();
        ...
        List<PartnerCorpHistory08Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory08Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory08ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory08ItemWriter.write(items);
