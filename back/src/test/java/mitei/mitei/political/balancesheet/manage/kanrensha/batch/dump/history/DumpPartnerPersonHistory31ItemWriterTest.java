...
/**
 * DumpPartnerPersonHistory31ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory31ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_31.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory31ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory31Entity entity00 = new PartnerPersonHistory31Entity();
        ...
        List<PartnerPersonHistory31Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory31Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory31ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory31ItemWriter.write(items);
