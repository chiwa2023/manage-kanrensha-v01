...
/**
 * DumpPartnerCorpHistory35ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory35ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_35.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory35ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory35Entity entity00 = new PartnerCorpHistory35Entity();
        ...
        List<PartnerCorpHistory35Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory35Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory35ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory35ItemWriter.write(items);
