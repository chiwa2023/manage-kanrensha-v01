...
/**
 * DumpPartnerPersonHistory30ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory30ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_30.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory30ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory30Entity entity00 = new PartnerPersonHistory30Entity();
        ...
        List<PartnerPersonHistory30Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory30Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory30ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory30ItemWriter.write(items);
