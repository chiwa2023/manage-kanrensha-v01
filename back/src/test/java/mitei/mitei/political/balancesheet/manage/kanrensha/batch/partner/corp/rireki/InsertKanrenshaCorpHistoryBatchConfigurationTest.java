...
/**
 * InsertKanrenshaCorpHistoryBatchConfiguration単体テスト
 */
@SpringJUnitConfig
...
class InsertKanrenshaCorpHistoryBatchConfigurationTest {
    ...
    /** 起動をするJob */
    @Qualifier(InsertKanrenshaCorpHistoryBatchConfiguration.JOB_NAME)
    @Autowired
    private Job insertKanrenshaCorpHistory;
    ...
    @Tag("TableTruncate")
    void testJob() {
        assertEquals(InsertKanrenshaCorpHistoryBatchConfiguration.JOB_NAME, insertKanrenshaCorpHistory.getName(),
                "Job名が一致");
    }

    @Test
    @Tag("TableTruncate")
    @Sql({ "sample_wk_tbl_partner_corp_history.sql", "sample_wk_tbl_partner_corp_judge.sql", "master_corporation02.sql",
            "sample_history01.sql" })
    void testExecute() throws Exception {

        jobLauncherTestUtils.setJob(insertKanrenshaCorpHistory);

        Path path = Paths.get(GetCurrentResourcePath.getBackTestResourcePath(), "batch/partner_corp", "関連者企業団体履歴.csv");

        JobParameters jobParameters = new JobParametersBuilder(
                insertKanrenshaCorpHistory.getJobParametersIncrementer().getNext(new JobParameters())) // NOPMD
