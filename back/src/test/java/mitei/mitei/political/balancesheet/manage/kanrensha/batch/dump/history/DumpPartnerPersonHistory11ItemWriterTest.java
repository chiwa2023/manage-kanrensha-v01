...
/**
 * DumpPartnerPersonHistory11ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory11ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_11.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory11ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory11Entity entity00 = new PartnerPersonHistory11Entity();
        ...
        List<PartnerPersonHistory11Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory11Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory11ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory11ItemWriter.write(items);
