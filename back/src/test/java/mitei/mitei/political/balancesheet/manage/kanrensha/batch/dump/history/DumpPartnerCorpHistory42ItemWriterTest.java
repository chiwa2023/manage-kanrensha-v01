...
/**
 * DumpPartnerCorpHistory42ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory42ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_42.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory42ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory42Entity entity00 = new PartnerCorpHistory42Entity();
        ...
        List<PartnerCorpHistory42Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory42Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory42ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory42ItemWriter.write(items);
