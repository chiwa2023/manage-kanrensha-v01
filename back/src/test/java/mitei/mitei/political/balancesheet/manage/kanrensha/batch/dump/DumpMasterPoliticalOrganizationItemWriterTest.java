...
/**
 * DumpMasterPoliticalOrganizationItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpMasterPoliticalOrganizationItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/master_poli_org.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpMasterPoliticalOrganizationItemWriter.beforeStep(stepExecution);

        MasterPoliticalOrganizationDto dto = new MasterPoliticalOrganizationDto();
        dto.setPoliOrgKanrenshaCode("123-4567");
        dto.setPartnerName("ちゃらんぽらん政治団体");
        dto.setAllAddress("和歌山県架空市山麓町");
        dto.setPoliOrgDelegate("組合長　花子");
        dto.setInsertTimestamp(LocalDateTime.of(2022, 12, 5, 1, 2, 3));

        List<MasterPoliticalOrganizationDto> list = new ArrayList<>();
        list.add(dto);

        // Chunkを作成してセット
        Chunk<? extends MasterPoliticalOrganizationDto> items = new Chunk<>(list);
        dumpMasterPoliticalOrganizationItemWriter.open(stepExecution.getExecutionContext());
        dumpMasterPoliticalOrganizationItemWriter.write(items);

        List<String> listAns = Files.readAllLines(path);

        assertEquals("\"123-4567\",\"ちゃらんぽらん政治団体\",\"和歌山県架空市山麓町\",\"組合長　花子\",\"2022-12-05T01:02:03\"", listAns.get(1));
    }
