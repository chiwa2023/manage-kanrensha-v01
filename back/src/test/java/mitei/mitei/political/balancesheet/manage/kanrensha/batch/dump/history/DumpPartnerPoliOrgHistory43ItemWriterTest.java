...
/**
 * DumpPartnerPoliOrgHistory43ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory43ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org43.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory43ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory43Entity entity00 = new PartnerPoliOrgHistory43Entity();
        ...
        List<PartnerPoliOrgHistory43Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory43Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory43ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory43ItemWriter.write(items);
