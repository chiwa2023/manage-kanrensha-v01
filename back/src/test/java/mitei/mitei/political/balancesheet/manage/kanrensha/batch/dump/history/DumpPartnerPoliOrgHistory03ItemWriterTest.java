...
/**
 * DumpPartnerPoliOrgHistory03ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory03ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org03.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory03ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory03Entity entity00 = new PartnerPoliOrgHistory03Entity();
        ...
        List<PartnerPoliOrgHistory03Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory03Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory03ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory03ItemWriter.write(items);
