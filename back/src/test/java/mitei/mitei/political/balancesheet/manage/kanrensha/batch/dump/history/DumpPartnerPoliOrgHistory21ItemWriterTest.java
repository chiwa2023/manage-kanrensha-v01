...
/**
 * DumpPartnerPoliOrgHistory21ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory21ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org21.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory21ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory21Entity entity00 = new PartnerPoliOrgHistory21Entity();
        ...
        List<PartnerPoliOrgHistory21Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory21Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory21ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory21ItemWriter.write(items);
