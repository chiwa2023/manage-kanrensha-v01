...
/**
 * DumpPartnerPersonHistory44ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory44ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_44.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory44ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory44Entity entity00 = new PartnerPersonHistory44Entity();
        ...
        List<PartnerPersonHistory44Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory44Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory44ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory44ItemWriter.write(items);
