...
/**
 * DumpPartnerPersonHistory19ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory19ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_19.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory19ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory19Entity entity00 = new PartnerPersonHistory19Entity();
        ...
        List<PartnerPersonHistory19Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory19Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory19ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory19ItemWriter.write(items);
