...
/**
 * DumpPartnerPersonHistory43ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory43ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_43.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory43ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory43Entity entity00 = new PartnerPersonHistory43Entity();
        ...
        List<PartnerPersonHistory43Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory43Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory43ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory43ItemWriter.write(items);
