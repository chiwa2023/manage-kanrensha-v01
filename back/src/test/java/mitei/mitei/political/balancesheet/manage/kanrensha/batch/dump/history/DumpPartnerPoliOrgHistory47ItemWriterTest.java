...
/**
 * DumpPartnerPoliOrgHistory47ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory47ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org47.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory47ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory47Entity entity00 = new PartnerPoliOrgHistory47Entity();
        ...
        List<PartnerPoliOrgHistory47Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory47Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory47ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory47ItemWriter.write(items);
