...
/**
 * PartnerCorpAddMiniCsvItemWriter単体テスト
 */
@SpringJUnitConfig
@AutoConfigureMockMvc
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT)
@DirtiesContext(classMode = ClassMode.BEFORE_CLASS)
class PartnerCorpAddMiniCsvItemWriterTest {
    ...
    void test() throws Exception {
        ...
        partnerCorpAddMiniCsvItemWriter.beforeStep(this.getStepExecution());
        partnerCorpAddMiniCsvItemWriter.write(items);

        List<WkTblPartnerCorpAddMinEntity> listAns = wkTblPartnerCorpAddMinRepository.findAll();
        assertEquals(1, listAns.size());

        WkTblPartnerCorpAddMinEntity entityAns = listAns.get(0);
