...
/**
 * DumpPartnerPersonHistory10ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory10ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_10.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory10ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory10Entity entity00 = new PartnerPersonHistory10Entity();
        ...
        List<PartnerPersonHistory10Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory10Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory10ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory10ItemWriter.write(items);
