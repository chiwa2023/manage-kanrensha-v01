...
/**
 * DumpPartnerPoliOrgHistory36ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory36ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org36.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory36ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory36Entity entity00 = new PartnerPoliOrgHistory36Entity();
        ...
        List<PartnerPoliOrgHistory36Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory36Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory36ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory36ItemWriter.write(items);
