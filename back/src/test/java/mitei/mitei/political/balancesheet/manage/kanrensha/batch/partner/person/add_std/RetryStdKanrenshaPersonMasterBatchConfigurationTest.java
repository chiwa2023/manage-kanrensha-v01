...
class RetryStdKanrenshaPersonMasterBatchConfigurationTest {
    ...
    @Tag("TableTruncate")
    void testJob() {
        assertEquals(RetryStdKanrenshaPersonMasterBatchConfiguration.JOB_NAME, retryStdKanrenshaPersonMaster.getName(),
                "Job名が一致");
    }

    @Test
    @Tag("TableTruncate")
    @Sql({ "sample_wk_tbl_master_person_judge.sql", "sample_wk_tbl_master_person.sql", "delete_master_person.sql",
            "delete_master_person_base.sql", "delete_master_person_address.sql", "delete_master_person_access.sql",
            "delete_master_person_property.sql", "delete_history_01.sql" })
    void testExecute() throws Exception {

        jobLauncherTestUtils.setJob(retryStdKanrenshaPersonMaster);
