...
/**
 * DumpPartnerPersonHistory27ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory27ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_27.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory27ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory27Entity entity00 = new PartnerPersonHistory27Entity();
        ...
        List<PartnerPersonHistory27Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory27Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory27ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory27ItemWriter.write(items);
