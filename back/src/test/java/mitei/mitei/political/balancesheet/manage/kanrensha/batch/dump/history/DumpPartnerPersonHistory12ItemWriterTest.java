...
/**
 * DumpPartnerPersonHistory12ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory12ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_12.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory12ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory12Entity entity00 = new PartnerPersonHistory12Entity();
        ...
        List<PartnerPersonHistory12Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory12Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory12ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory12ItemWriter.write(items);
