...
/**
 * DumpPartnerCorpHistory25ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory25ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_25.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory25ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory25Entity entity00 = new PartnerCorpHistory25Entity();
        ...
        List<PartnerCorpHistory25Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory25Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory25ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory25ItemWriter.write(items);
