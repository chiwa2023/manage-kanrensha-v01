...
/**
 * DumpPartnerCorpHistory47ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory47ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_47.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory47ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory47Entity entity00 = new PartnerCorpHistory47Entity();
        ...
        List<PartnerCorpHistory47Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory47Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory47ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory47ItemWriter.write(items);
