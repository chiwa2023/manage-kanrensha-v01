...
/**
 * DumpPartnerPersonHistory15ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory15ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_15.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory15ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory15Entity entity00 = new PartnerPersonHistory15Entity();
        ...
        List<PartnerPersonHistory15Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory15Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory15ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory15ItemWriter.write(items);
