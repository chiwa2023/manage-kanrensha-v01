...
/**
 * DumpPartnerPersonHistory04ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory04ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_04.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory04ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory04Entity entity00 = new PartnerPersonHistory04Entity();
        ...
        List<PartnerPersonHistory04Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory04Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory04ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory04ItemWriter.write(items);
