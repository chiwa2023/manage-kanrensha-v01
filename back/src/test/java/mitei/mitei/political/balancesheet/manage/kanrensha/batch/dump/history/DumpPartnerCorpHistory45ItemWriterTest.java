...
/**
 * DumpPartnerCorpHistory45ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory45ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_45.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory45ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory45Entity entity00 = new PartnerCorpHistory45Entity();
        ...
        List<PartnerCorpHistory45Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory45Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory45ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory45ItemWriter.write(items);
