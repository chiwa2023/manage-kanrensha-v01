...
/**
 * DumpPartnerPersonHistory02ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory02ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_02.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory02ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory02Entity entity00 = new PartnerPersonHistory02Entity();
        ...
        List<PartnerPersonHistory02Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory02Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory02ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory02ItemWriter.write(items);
