...
/**
 * DumpPartnerCorpHistory28ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory28ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_28.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory28ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory28Entity entity00 = new PartnerCorpHistory28Entity();
        ...
        List<PartnerCorpHistory28Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory28Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory28ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory28ItemWriter.write(items);
