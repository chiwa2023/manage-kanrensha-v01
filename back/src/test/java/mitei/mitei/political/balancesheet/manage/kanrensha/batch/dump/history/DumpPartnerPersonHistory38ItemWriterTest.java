...
/**
 * DumpPartnerPersonHistory38ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory38ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_38.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory38ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory38Entity entity00 = new PartnerPersonHistory38Entity();
        ...
        List<PartnerPersonHistory38Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory38Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory38ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory38ItemWriter.write(items);
