...
/**
 * DumpPartnerCorpHistory19ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory19ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_19.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory19ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory19Entity entity00 = new PartnerCorpHistory19Entity();
        ...
        List<PartnerCorpHistory19Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory19Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory19ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory19ItemWriter.write(items);
