...
/**
 * DumpPartnerCorpHistory14ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory14ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_14.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory14ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory14Entity entity00 = new PartnerCorpHistory14Entity();
        ...
        List<PartnerCorpHistory14Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory14Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory14ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory14ItemWriter.write(items);
