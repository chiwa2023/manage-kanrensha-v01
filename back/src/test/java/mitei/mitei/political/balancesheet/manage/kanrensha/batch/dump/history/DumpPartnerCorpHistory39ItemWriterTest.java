...
/**
 * DumpPartnerCorpHistory39ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory39ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_39.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory39ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory39Entity entity00 = new PartnerCorpHistory39Entity();
        ...
        List<PartnerCorpHistory39Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory39Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory39ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory39ItemWriter.write(items);
