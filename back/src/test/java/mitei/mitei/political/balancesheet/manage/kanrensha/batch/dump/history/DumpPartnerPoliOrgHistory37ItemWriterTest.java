...
/**
 * DumpPartnerPoliOrgHistory37ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory37ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org37.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory37ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory37Entity entity00 = new PartnerPoliOrgHistory37Entity();
        ...
        List<PartnerPoliOrgHistory37Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory37Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory37ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory37ItemWriter.write(items);
