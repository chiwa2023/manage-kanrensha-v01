...
/**
 * DumpPartnerPersonHistory45ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory45ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_45.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory45ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory45Entity entity00 = new PartnerPersonHistory45Entity();
        ...
        List<PartnerPersonHistory45Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory45Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory45ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory45ItemWriter.write(items);
