...
/**
 * DumpPartnerPoliOrgHistory38ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory38ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org38.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory38ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory38Entity entity00 = new PartnerPoliOrgHistory38Entity();
        ...
        List<PartnerPoliOrgHistory38Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory38Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory38ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory38ItemWriter.write(items);
