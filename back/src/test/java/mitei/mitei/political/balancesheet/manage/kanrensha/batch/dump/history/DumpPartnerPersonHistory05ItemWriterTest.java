...
/**
 * DumpPartnerPersonHistory05ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory05ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_05.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory05ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory05Entity entity00 = new PartnerPersonHistory05Entity();
        ...
        List<PartnerPersonHistory05Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory05Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory05ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory05ItemWriter.write(items);
