...
/**
 * DumpPartnerCorpHistory22ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory22ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_22.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory22ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory22Entity entity00 = new PartnerCorpHistory22Entity();
        ...
        List<PartnerCorpHistory22Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory22Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory22ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory22ItemWriter.write(items);
