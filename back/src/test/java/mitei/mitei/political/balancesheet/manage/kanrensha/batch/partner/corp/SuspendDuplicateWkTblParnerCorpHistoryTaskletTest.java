...
/**
 * SuspendDuplicateWkTblParnerCorpHistoryTasklet単体テスト
 */
@SpringJUnitConfig
@AutoConfigureMockMvc
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT)
@DirtiesContext(classMode = ClassMode.BEFORE_CLASS)
class SuspendDuplicateWkTblParnerCorpHistoryTaskletTest {
    // CHECKSTYLE:OFF
    ...
    @Tag("TableTruncate")
    @Transactional
    @Sql("duplicatee_wk_tbl_partner_corp_history.sql")
    void test() throws Exception {
        final String reason = "アップロードファイル内で重複しているデータです";

        suspendDuplicateWkTblParnerCorpHistoryTasklet.beforeStep(this.getStepExecution());
        suspendDuplicateWkTblParnerCorpHistoryTasklet.execute(null, null);

        WkTblPartnerCorpHistoryEntity entity501 = wkTblPartnerCorpHistoryRepository.findById(501).get();
        ...
        WkTblPartnerCorpHistoryEntity entity502 = wkTblPartnerCorpHistoryRepository.findById(502).get();
        ...
        WkTblPartnerCorpHistoryEntity entity601 = wkTblPartnerCorpHistoryRepository.findById(601).get();
        ...
        assertEquals(true, wkTblPartnerCorpHistoryRepository.findById(101).get().getIsLatest());
        assertEquals(true, wkTblPartnerCorpHistoryRepository.findById(102).get().getIsLatest());
        assertEquals(true, wkTblPartnerCorpHistoryRepository.findById(103).get().getIsLatest());
   }
