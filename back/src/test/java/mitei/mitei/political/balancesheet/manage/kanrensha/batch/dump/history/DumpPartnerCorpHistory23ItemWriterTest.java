...
/**
 * DumpPartnerCorpHistory23ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory23ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_23.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory23ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory23Entity entity00 = new PartnerCorpHistory23Entity();
        ...
        List<PartnerCorpHistory23Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory23Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory23ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory23ItemWriter.write(items);
