...
/**
 * DumpPartnerPoliOrgHistory42ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory42ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org42.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory42ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory42Entity entity00 = new PartnerPoliOrgHistory42Entity();
        ...
        List<PartnerPoliOrgHistory42Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory42Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory42ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory42ItemWriter.write(items);
