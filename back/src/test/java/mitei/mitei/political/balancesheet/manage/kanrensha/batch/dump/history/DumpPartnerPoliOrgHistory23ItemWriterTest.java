...
/**
 * DumpPartnerPoliOrgHistory23ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory23ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org23.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory23ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory23Entity entity00 = new PartnerPoliOrgHistory23Entity();
        ...
        List<PartnerPoliOrgHistory23Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory23Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory23ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory23ItemWriter.write(items);
