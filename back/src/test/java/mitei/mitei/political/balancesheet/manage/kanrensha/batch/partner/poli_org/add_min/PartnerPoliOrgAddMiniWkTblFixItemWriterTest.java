...
/**
 * PartnerPoliOrgAddMiniWkTblFixItemWriter単体テスト
 */
@SpringJUnitConfig
@AutoConfigureMockMvc
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT)
@DirtiesContext(classMode = ClassMode.BEFORE_CLASS)
class PartnerPoliOrgAddMiniWkTblFixItemWriterTest {
    ...
    void test() throws Exception {
        ...
        final int executeId = 303;
        WkTblPartnerPoliOrgAddMinEntity entityCall = wkTblPartnerPoliOrgAddMinRepository.findById(executeId).get();
        entityCall.setIsFinish(true);
        entityCall.setJudgeReason("正常終了");

        List<WkTblPartnerPoliOrgAddMinEntity> list = new ArrayList<>();
        list.add(entityCall);

        // Chunkを作成してセット
        Chunk<? extends WkTblPartnerPoliOrgAddMinEntity> items = new Chunk<>(list);

        partnerPoliOrgAddMiniWkTblFixItemWriter.beforeStep(this.getStepExecution());
        partnerPoliOrgAddMiniWkTblFixItemWriter.write(items);

        WkTblPartnerPoliOrgAddMinEntity entityAns = wkTblPartnerPoliOrgAddMinRepository.findById(executeId).get();
