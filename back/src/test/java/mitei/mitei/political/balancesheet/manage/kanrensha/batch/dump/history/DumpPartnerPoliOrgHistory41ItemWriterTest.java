...
/**
 * DumpPartnerPoliOrgHistory41ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory41ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org41.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory41ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory41Entity entity00 = new PartnerPoliOrgHistory41Entity();
        ...
        List<PartnerPoliOrgHistory41Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory41Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory41ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory41ItemWriter.write(items);
