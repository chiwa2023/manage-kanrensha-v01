...
/**
 * DumpPartnerCorpHistory05ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory05ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_05.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory05ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory05Entity entity00 = new PartnerCorpHistory05Entity();
        ...
        List<PartnerCorpHistory05Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory05Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory05ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory05ItemWriter.write(items);
