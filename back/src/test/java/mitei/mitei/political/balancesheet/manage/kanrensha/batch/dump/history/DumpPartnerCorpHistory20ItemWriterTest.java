...
/**
 * DumpPartnerCorpHistory20ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory20ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_20.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory20ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory20Entity entity00 = new PartnerCorpHistory20Entity();
        ...
        List<PartnerCorpHistory20Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory20Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory20ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory20ItemWriter.write(items);
