...
/**
 * DumpPartnerCorpHistory03ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory03ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_03.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory03ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory03Entity entity00 = new PartnerCorpHistory03Entity();
        ...
        List<PartnerCorpHistory03Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory03Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory03ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory03ItemWriter.write(items);
