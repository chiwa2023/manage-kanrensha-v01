...
/**
 * DumpPartnerPoliOrgHistory34ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory34ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org34.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory34ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory34Entity entity00 = new PartnerPoliOrgHistory34Entity();
        ...
        List<PartnerPoliOrgHistory34Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory34Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory34ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory34ItemWriter.write(items);
