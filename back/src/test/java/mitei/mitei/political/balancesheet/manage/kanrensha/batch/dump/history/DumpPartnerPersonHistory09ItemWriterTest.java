...
/**
 * DumpPartnerPersonHistory09ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory09ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_09.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory09ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory09Entity entity00 = new PartnerPersonHistory09Entity();
        ...
        List<PartnerPersonHistory09Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory09Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory09ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory09ItemWriter.write(items);
