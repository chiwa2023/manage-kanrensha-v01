...
/**
 * DumpPartnerCorpHistory27ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory27ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_27.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory27ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory27Entity entity00 = new PartnerCorpHistory27Entity();
        ...
        List<PartnerCorpHistory27Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory27Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory27ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory27ItemWriter.write(items);
