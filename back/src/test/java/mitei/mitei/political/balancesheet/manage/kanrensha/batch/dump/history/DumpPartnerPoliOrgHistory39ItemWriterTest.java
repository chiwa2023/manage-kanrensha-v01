...
/**
 * DumpPartnerPoliOrgHistory39ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory39ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org39.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory39ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory39Entity entity00 = new PartnerPoliOrgHistory39Entity();
        ...
        List<PartnerPoliOrgHistory39Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory39Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory39ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory39ItemWriter.write(items);
