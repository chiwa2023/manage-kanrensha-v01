...
/**
 * DumpPartnerPersonHistory22ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory22ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_22.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory22ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory22Entity entity00 = new PartnerPersonHistory22Entity();
        ...
        List<PartnerPersonHistory22Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory22Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory22ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory22ItemWriter.write(items);
