...
/**
 * DumpPartnerCorpHistory32ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory32ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_32.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory32ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory32Entity entity00 = new PartnerCorpHistory32Entity();
        ...
        List<PartnerCorpHistory32Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory32Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory32ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory32ItemWriter.write(items);
