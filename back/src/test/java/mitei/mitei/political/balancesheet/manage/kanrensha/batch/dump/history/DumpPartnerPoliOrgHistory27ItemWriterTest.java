...
/**
 * DumpPartnerPoliOrgHistory27ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory27ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org27.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory27ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory27Entity entity00 = new PartnerPoliOrgHistory27Entity();
        ...
        List<PartnerPoliOrgHistory27Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory27Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory27ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory27ItemWriter.write(items);
