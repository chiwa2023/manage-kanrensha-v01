...
/**
 * DumpPartnerPersonHistory16ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory16ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_16.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory16ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory16Entity entity00 = new PartnerPersonHistory16Entity();
        ...
        List<PartnerPersonHistory16Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory16Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory16ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory16ItemWriter.write(items);
