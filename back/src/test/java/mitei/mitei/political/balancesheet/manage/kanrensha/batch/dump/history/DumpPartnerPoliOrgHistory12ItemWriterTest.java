...
/**
 * DumpPartnerPoliOrgHistory12ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory12ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org12.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory12ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory12Entity entity00 = new PartnerPoliOrgHistory12Entity();
        ...
        List<PartnerPoliOrgHistory12Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory12Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory12ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory12ItemWriter.write(items);
