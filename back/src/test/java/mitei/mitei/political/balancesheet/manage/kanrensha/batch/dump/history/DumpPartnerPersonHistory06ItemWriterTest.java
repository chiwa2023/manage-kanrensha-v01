...
/**
 * DumpPartnerPersonHistory06ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory06ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_06.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory06ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory06Entity entity00 = new PartnerPersonHistory06Entity();
        ...
        List<PartnerPersonHistory06Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory06Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory06ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory06ItemWriter.write(items);
