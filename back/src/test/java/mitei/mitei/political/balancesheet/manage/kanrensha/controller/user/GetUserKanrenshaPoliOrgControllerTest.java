...
/**
 * GetUserKanrenshaPoliOrgController単体テスト
 */
@SpringJUnitConfig
@AutoConfigureMockMvc
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT)
@DirtiesContext(classMode = ClassMode.BEFORE_CLASS)
class GetUserKanrenshaPoliOrgControllerTest {
    // CHECKSTYLE:OFF
    ...
    @Transactional
    @WithMockUser
    @Sql("../../service/kanrensha/EditKanrenshaPoliOrgServiceTest.sql")
    void test() throws Exception {

        MasterPoliticalOrganizationEntity entityMaster = masterPoliticalOrganizationRepository.findById(724).get();
        GetKanrenshaPoliOrgCapsuleDto capsuleDto = new GetKanrenshaPoliOrgCapsuleDto();
        capsuleDto.setMasterPoliticalOrganizationEntity(entityMaster);
        capsuleDto.setUserPersonLeastDto(CreateLeastUserForTestUtil.practice());

        ObjectMapper objectMapper = GetObjectMapperWithTimeModuleUtil.practice();

        String path = "/user-kanrensha/get-poli-org";
