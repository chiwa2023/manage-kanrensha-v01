...
/**
 * DumpPartnerPoliOrgHistory20ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory20ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org20.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory20ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory20Entity entity00 = new PartnerPoliOrgHistory20Entity();
        ...
        List<PartnerPoliOrgHistory20Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory20Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory20ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory20ItemWriter.write(items);
