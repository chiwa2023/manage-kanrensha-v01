...
/**
 * DumpPartnerCorpHistory40ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory40ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_40.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory40ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory40Entity entity00 = new PartnerCorpHistory40Entity();
        ...
        List<PartnerCorpHistory40Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory40Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory40ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory40ItemWriter.write(items);
