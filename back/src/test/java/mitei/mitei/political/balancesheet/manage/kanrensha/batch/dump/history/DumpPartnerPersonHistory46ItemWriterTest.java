...
/**
 * DumpPartnerPersonHistory46ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory46ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_46.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory46ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory46Entity entity00 = new PartnerPersonHistory46Entity();
        ...
        List<PartnerPersonHistory46Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory46Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory46ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory46ItemWriter.write(items);
