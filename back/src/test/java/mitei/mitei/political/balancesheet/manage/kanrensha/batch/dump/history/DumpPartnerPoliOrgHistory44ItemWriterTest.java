...
/**
 * DumpPartnerPoliOrgHistory44ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory44ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org44.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory44ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory44Entity entity00 = new PartnerPoliOrgHistory44Entity();
        ...
        List<PartnerPoliOrgHistory44Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory44Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory44ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory44ItemWriter.write(items);
