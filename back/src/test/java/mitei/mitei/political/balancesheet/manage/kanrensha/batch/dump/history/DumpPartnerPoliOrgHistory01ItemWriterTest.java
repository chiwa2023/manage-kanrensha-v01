...
/**
 * DumpPartnerPoliOrgHistory01ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory01ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org01.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory01ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory01Entity entity00 = new PartnerPoliOrgHistory01Entity();
        ...
        List<PartnerPoliOrgHistory01Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory01Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory01ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory01ItemWriter.write(items);
