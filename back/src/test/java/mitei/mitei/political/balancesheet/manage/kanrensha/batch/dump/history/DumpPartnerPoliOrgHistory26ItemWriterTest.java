...
/**
 * DumpPartnerPoliOrgHistory26ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory26ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org26.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory26ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory26Entity entity00 = new PartnerPoliOrgHistory26Entity();
        ...
        List<PartnerPoliOrgHistory26Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory26Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory26ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory26ItemWriter.write(items);
