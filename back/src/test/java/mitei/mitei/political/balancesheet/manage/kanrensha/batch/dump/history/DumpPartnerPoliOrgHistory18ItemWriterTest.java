...
/**
 * DumpPartnerPoliOrgHistory18ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory18ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org18.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory18ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory18Entity entity00 = new PartnerPoliOrgHistory18Entity();
        ...
        List<PartnerPoliOrgHistory18Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory18Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory18ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory18ItemWriter.write(items);
