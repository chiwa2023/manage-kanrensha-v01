...
/**
 * DumpPartnerPoliOrgHistory99ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory99ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org99.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory99ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory99Entity entity00 = new PartnerPoliOrgHistory99Entity();
        ...
        List<PartnerPoliOrgHistory99Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory99Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory99ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory99ItemWriter.write(items);
