...
/**
 * DumpPartnerPoliOrgHistory46ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory46ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org46.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory46ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory46Entity entity00 = new PartnerPoliOrgHistory46Entity();
        ...
        List<PartnerPoliOrgHistory46Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory46Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory46ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory46ItemWriter.write(items);
