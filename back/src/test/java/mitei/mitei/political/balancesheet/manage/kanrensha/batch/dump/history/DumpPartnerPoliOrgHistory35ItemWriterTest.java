...
/**
 * DumpPartnerPoliOrgHistory35ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory35ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org35.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory35ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory35Entity entity00 = new PartnerPoliOrgHistory35Entity();
        ...
        List<PartnerPoliOrgHistory35Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory35Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory35ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory35ItemWriter.write(items);
