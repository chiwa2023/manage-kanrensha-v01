...
/**
 * DumpPartnerPoliOrgHistory11ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory11ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org11.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory11ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory11Entity entity00 = new PartnerPoliOrgHistory11Entity();
        ...
        List<PartnerPoliOrgHistory11Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory11Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory11ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory11ItemWriter.write(items);
