...
/**
 * DumpPartnerCorpHistory07ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory07ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_07.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory07ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory07Entity entity00 = new PartnerCorpHistory07Entity();
        ...
        List<PartnerCorpHistory07Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory07Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory07ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory07ItemWriter.write(items);
