...
/**
 * DumpPartnerPersonHistory26ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory26ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_26.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory26ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory26Entity entity00 = new PartnerPersonHistory26Entity();
        ...
        List<PartnerPersonHistory26Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory26Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory26ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory26ItemWriter.write(items);
