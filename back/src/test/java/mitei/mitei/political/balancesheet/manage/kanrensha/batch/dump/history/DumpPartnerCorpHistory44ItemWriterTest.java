...
/**
 * DumpPartnerCorpHistory44ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory44ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_44.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory44ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory44Entity entity00 = new PartnerCorpHistory44Entity();
        ...
        List<PartnerCorpHistory44Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory44Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory44ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory44ItemWriter.write(items);
