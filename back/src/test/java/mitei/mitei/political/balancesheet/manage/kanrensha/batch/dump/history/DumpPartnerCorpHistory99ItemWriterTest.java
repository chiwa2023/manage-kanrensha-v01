...
/**
 * DumpPartnerCorpHistory99ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory99ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_99.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory99ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory99Entity entity00 = new PartnerCorpHistory99Entity();
        ...
        List<PartnerCorpHistory99Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory99Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory99ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory99ItemWriter.write(items);
