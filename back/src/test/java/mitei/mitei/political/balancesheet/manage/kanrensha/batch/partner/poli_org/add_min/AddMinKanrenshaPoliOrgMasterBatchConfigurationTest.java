...
/**
 * AddMinKanrenshaPoliOrgMasterBatchConfiguration単体テスト
 */
@SpringJUnitConfig
...
class AddMinKanrenshaPoliOrgMasterBatchConfigurationTest {
    ...
    /** 起動をするJob */
    @Qualifier(AddMinKanrenshaPoliOrgMasterBatchConfiguration.JOB_NAME)
    @Autowired
    private Job addMinKanrenshaPoliOrgMaster;
    ...
    @Tag("TableTruncate")
    void testJob() {
        assertEquals(AddMinKanrenshaPoliOrgMasterBatchConfiguration.JOB_NAME, addMinKanrenshaPoliOrgMaster.getName(),
                "Job名が一致");
    }
    ...
    void testExecute() throws Exception {
        ...
        jobLauncherTestUtils.setJob(addMinKanrenshaPoliOrgMaster);

        Path path = Paths.get(GetCurrentResourcePath.getBackTestResourcePath(), "batch/partner_poli_org",
                "関連者政治団体最小登録.csv");

        JobParameters jobParameters = new JobParametersBuilder(
                addMinKanrenshaPoliOrgMaster.getJobParametersIncrementer().getNext(new JobParameters())) // NOPMD
