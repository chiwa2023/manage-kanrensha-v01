...
/**
 * DumpPartnerCorpHistory37ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory37ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_37.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory37ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory37Entity entity00 = new PartnerCorpHistory37Entity();
        ...
        List<PartnerCorpHistory37Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory37Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory37ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory37ItemWriter.write(items);
