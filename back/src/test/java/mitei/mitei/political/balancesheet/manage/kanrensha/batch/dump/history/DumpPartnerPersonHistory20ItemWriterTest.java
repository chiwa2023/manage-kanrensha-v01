...
/**
 * DumpPartnerPersonHistory20ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory20ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_20.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory20ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory20Entity entity00 = new PartnerPersonHistory20Entity();
        ...
        List<PartnerPersonHistory20Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory20Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory20ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory20ItemWriter.write(items);
