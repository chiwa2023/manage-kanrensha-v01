...
/**
 * DumpPartnerCorpHistory21ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory21ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_21.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory21ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory21Entity entity00 = new PartnerCorpHistory21Entity();
        ...
        List<PartnerCorpHistory21Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory21Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory21ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory21ItemWriter.write(items);
