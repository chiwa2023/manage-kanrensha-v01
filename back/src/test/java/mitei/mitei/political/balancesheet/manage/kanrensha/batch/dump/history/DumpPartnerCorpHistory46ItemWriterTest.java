...
/**
 * DumpPartnerCorpHistory46ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory46ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_46.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory46ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory46Entity entity00 = new PartnerCorpHistory46Entity();
        ...
        List<PartnerCorpHistory46Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory46Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory46ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory46ItemWriter.write(items);
