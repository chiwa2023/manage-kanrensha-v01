...
/**
 * DumpPartnerPersonHistory14ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory14ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_14.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory14ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory14Entity entity00 = new PartnerPersonHistory14Entity();
        ...
        List<PartnerPersonHistory14Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory14Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory14ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory14ItemWriter.write(items);
