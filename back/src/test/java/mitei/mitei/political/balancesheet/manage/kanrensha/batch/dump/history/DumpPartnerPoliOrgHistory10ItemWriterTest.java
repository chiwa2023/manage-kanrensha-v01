...
/**
 * DumpPartnerPoliOrgHistory10ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory10ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org10.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory10ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory10Entity entity00 = new PartnerPoliOrgHistory10Entity();
        ...
        List<PartnerPoliOrgHistory10Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory10Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory10ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory10ItemWriter.write(items);
