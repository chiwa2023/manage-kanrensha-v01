...
/**
 * DumpPartnerCorpHistory06ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory06ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_06.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory06ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory06Entity entity00 = new PartnerCorpHistory06Entity();
        ...
        List<PartnerCorpHistory06Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory06Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory06ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory06ItemWriter.write(items);
