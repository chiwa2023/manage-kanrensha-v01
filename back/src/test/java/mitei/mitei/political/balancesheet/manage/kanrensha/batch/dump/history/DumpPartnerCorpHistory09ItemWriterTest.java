...
/**
 * DumpPartnerCorpHistory09ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory09ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_09.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory09ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory09Entity entity00 = new PartnerCorpHistory09Entity();
        ...
        List<PartnerCorpHistory09Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory09Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory09ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory09ItemWriter.write(items);
