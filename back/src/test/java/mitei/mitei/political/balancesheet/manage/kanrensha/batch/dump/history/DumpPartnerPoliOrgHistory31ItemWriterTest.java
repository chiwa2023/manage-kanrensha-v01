...
/**
 * DumpPartnerPoliOrgHistory31ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory31ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org31.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory31ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory31Entity entity00 = new PartnerPoliOrgHistory31Entity();
        ...
        List<PartnerPoliOrgHistory31Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory31Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory31ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory31ItemWriter.write(items);
