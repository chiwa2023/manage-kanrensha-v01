...
/**
 * DumpPartnerCorpHistory33ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory33ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_33.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory33ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory33Entity entity00 = new PartnerCorpHistory33Entity();
        ...
        List<PartnerCorpHistory33Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory33Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory33ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory33ItemWriter.write(items);
