...
/**
 * DumpPartnerCorpHistory10ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory10ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_10.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory10ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory10Entity entity00 = new PartnerCorpHistory10Entity();
        ...
        List<PartnerCorpHistory10Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory10Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory10ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory10ItemWriter.write(items);
