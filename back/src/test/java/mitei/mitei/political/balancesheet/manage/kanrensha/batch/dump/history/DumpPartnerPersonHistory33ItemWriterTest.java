...
/**
 * DumpPartnerPersonHistory33ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory33ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_33.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory33ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory33Entity entity00 = new PartnerPersonHistory33Entity();
        ...
        List<PartnerPersonHistory33Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory33Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory33ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory33ItemWriter.write(items);
