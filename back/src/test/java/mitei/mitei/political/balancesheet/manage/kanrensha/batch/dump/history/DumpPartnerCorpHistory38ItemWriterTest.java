...
/**
 * DumpPartnerCorpHistory38ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory38ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_38.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory38ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory38Entity entity00 = new PartnerCorpHistory38Entity();
        ...
        List<PartnerCorpHistory38Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory38Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory38ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory38ItemWriter.write(items);
