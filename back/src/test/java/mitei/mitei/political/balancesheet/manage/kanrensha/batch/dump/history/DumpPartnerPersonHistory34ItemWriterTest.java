...
/**
 * DumpPartnerPersonHistory34ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory34ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_34.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory34ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory34Entity entity00 = new PartnerPersonHistory34Entity();
        ...
        List<PartnerPersonHistory34Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory34Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory34ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory34ItemWriter.write(items);
