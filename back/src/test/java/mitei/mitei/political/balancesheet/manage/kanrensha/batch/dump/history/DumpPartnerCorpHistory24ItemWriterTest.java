...
/**
 * DumpPartnerCorpHistory24ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory24ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_24.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory24ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory24Entity entity00 = new PartnerCorpHistory24Entity();
        ...
        List<PartnerCorpHistory24Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory24Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory24ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory24ItemWriter.write(items);
