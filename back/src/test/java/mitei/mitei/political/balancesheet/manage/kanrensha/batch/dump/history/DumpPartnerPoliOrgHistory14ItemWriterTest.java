...
/**
 * DumpPartnerPoliOrgHistory14ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory14ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org14.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory14ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory14Entity entity00 = new PartnerPoliOrgHistory14Entity();
        ...
        List<PartnerPoliOrgHistory14Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory14Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory14ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory14ItemWriter.write(items);
