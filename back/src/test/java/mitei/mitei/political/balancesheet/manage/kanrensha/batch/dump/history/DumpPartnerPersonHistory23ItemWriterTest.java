...
/**
 * DumpPartnerPersonHistory23ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory23ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_23.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory23ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory23Entity entity00 = new PartnerPersonHistory23Entity();
        ...
        List<PartnerPersonHistory23Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory23Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory23ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory23ItemWriter.write(items);
