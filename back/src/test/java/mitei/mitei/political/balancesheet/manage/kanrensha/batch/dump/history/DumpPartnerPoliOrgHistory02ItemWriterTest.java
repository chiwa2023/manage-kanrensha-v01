...
/**
 * DumpPartnerPoliOrgHistory02ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory02ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org02.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory02ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory02Entity entity00 = new PartnerPoliOrgHistory02Entity();
        ...
        List<PartnerPoliOrgHistory02Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory02Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory02ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory02ItemWriter.write(items);
