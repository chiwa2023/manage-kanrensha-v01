...
/**
 * SuspendDuplicateWkTblParnerPersonHistoryTasklet単体テスト
 */
@SpringJUnitConfig
@AutoConfigureMockMvc
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT)
@DirtiesContext(classMode = ClassMode.BEFORE_CLASS)
class SuspendDuplicateWkTblParnerPersonHistoryTaskletTest {
    // CHECKSTYLE:OFF
    ...
    @Tag("TableTruncate")
    @Transactional
    @Sql("duplicatee_wk_tbl_partner_person_history.sql")
    void test() throws Exception {

        final String reason = "アップロードファイル内で重複しているデータです";

        suspendDuplicateWkTblParnerPersonHistoryTasklet.beforeStep(this.getStepExecution());
        suspendDuplicateWkTblParnerPersonHistoryTasklet.execute(null, null);

        WkTblPartnerPersonHistoryEntity entity501 = wkTblPartnerPersonHistoryRepository.findById(501).get();
        ...
        WkTblPartnerPersonHistoryEntity entity502 = wkTblPartnerPersonHistoryRepository.findById(502).get();
        ...
        WkTblPartnerPersonHistoryEntity entity601 = wkTblPartnerPersonHistoryRepository.findById(601).get();
        ...
        assertEquals(true, wkTblPartnerPersonHistoryRepository.findById(98).get().getIsLatest());
        assertEquals(true, wkTblPartnerPersonHistoryRepository.findById(99).get().getIsLatest());
        assertEquals(true, wkTblPartnerPersonHistoryRepository.findById(100).get().getIsLatest());
