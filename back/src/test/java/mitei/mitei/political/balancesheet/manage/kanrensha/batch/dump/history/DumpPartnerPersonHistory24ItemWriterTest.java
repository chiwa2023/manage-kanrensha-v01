...
/**
 * DumpPartnerPersonHistory24ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory24ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_24.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory24ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory24Entity entity00 = new PartnerPersonHistory24Entity();
        entity00.setPartnerName("個人名");
        entity00.setAllAddress("全住所");
        entity00.setPersonShokugyou("個人職業");
        entity00.setPersonKanrenshaCode("123-4556");
        entity00.setInsertTimestamp(LocalDateTime.of(2022, 12, 5, 1, 2, 3));

        List<PartnerPersonHistory24Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory24Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory24ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory24ItemWriter.write(items);

        List<String> listAns = Files.readAllLines(path);

        assertEquals("\"123-4556\",\"個人名\",\"全住所\",\"個人職業\",\"2022-12-05T01:02:03\"", listAns.get(1));
