...
/**
 * DumpPartnerCorpHistory18ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory18ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_18.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory18ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory18Entity entity00 = new PartnerCorpHistory18Entity();
        ...
        List<PartnerCorpHistory18Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory18Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory18ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory18ItemWriter.write(items);
