...
/**
 * DumpPartnerPersonHistory01ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory01ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_01.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory01ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory01Entity entity00 = new PartnerPersonHistory01Entity();
        ...
        List<PartnerPersonHistory01Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory01Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory01ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory01ItemWriter.write(items);
