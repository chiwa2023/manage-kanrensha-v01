...
/**
 * DumpPartnerPoliOrgHistory25ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory25ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org25.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory25ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory25Entity entity00 = new PartnerPoliOrgHistory25Entity();
        ...
        List<PartnerPoliOrgHistory25Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory25Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory25ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory25ItemWriter.write(items);
