...
/**
 * DumpPartnerCorpHistory01ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory01ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_01.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory01ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory01Entity entity00 = new PartnerCorpHistory01Entity();
        ...
        List<PartnerCorpHistory01Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory01Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory01ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory01ItemWriter.write(items);
