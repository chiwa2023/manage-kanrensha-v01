...
/**
 * DumpPartnerPersonHistory08ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory08ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_08.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory08ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory08Entity entity00 = new PartnerPersonHistory08Entity();
        ...
        List<PartnerPersonHistory08Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory08Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory08ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory08ItemWriter.write(items);
