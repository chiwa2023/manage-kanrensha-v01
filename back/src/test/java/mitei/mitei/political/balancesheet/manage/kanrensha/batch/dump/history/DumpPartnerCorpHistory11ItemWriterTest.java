...
/**
 * DumpPartnerCorpHistory11ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory11ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_11.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory11ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory11Entity entity00 = new PartnerCorpHistory11Entity();
        ...
        List<PartnerCorpHistory11Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory11Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory11ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory11ItemWriter.write(items);
