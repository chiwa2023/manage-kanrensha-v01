...
/**
 * DumpPartnerCorpHistory17ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory17ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_17.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory17ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory17Entity entity00 = new PartnerCorpHistory17Entity();
        ...
        List<PartnerCorpHistory17Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory17Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory17ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory17ItemWriter.write(items);
