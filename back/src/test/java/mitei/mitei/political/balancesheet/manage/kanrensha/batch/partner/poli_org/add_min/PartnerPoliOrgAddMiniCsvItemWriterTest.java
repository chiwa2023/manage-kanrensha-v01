...
/**
 * PartnerPoliOrgAddMiniCsvItemWriter単体テスト
 */
@SpringJUnitConfig
@AutoConfigureMockMvc
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT)
@DirtiesContext(classMode = ClassMode.BEFORE_CLASS)
class PartnerPoliOrgAddMiniCsvItemWriterTest {
    ...
    void test() throws Exception {
        ...
        partnerPoliOrgAddMiniCsvItemWriter.beforeStep(this.getStepExecution());
        partnerPoliOrgAddMiniCsvItemWriter.write(items);

        List<WkTblPartnerPoliOrgAddMinEntity> listAns = wkTblPartnerPoliOrgAddMinRepository.findAll();
        assertEquals(1, listAns.size());

        WkTblPartnerPoliOrgAddMinEntity entityAns = listAns.get(0);
