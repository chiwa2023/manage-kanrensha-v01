...
/**
 * DumpPartnerPersonHistory35ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory35ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_35.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory35ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory35Entity entity00 = new PartnerPersonHistory35Entity();
        ...
        List<PartnerPersonHistory35Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory35Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory35ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory35ItemWriter.write(items);
