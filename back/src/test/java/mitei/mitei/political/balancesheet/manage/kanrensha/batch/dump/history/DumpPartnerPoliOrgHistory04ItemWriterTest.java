...
/**
 * DumpPartnerPoliOrgHistory04ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory04ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org04.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory04ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory04Entity entity00 = new PartnerPoliOrgHistory04Entity();
        ...
        List<PartnerPoliOrgHistory04Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory04Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory04ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory04ItemWriter.write(items);
