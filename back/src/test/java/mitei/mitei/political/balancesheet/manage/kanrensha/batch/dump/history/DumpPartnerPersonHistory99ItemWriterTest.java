...
/**
 * DumpPartnerPersonHistory99ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory99ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_99.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory99ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory99Entity entity00 = new PartnerPersonHistory99Entity();
        ...
        List<PartnerPersonHistory99Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory99Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory99ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory99ItemWriter.write(items);
