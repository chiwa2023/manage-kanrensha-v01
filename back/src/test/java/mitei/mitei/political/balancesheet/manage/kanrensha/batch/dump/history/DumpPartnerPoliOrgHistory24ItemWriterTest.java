...
/**
 * DumpPartnerPoliOrgHistory24ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory24ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org24.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory24ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory24Entity entity00 = new PartnerPoliOrgHistory24Entity();
        ...
        List<PartnerPoliOrgHistory24Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory24Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory24ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory24ItemWriter.write(items);
