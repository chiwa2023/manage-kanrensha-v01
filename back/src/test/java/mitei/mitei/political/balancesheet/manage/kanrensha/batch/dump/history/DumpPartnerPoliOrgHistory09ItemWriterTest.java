...
/**
 * DumpPartnerPoliOrgHistory09ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory09ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org09.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory09ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory09Entity entity00 = new PartnerPoliOrgHistory09Entity();
        ...
        List<PartnerPoliOrgHistory09Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory09Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory09ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory09ItemWriter.write(items);
