...
/**
 * DumpPartnerPoliOrgHistory29ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory29ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org29.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory29ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory29Entity entity00 = new PartnerPoliOrgHistory29Entity();
        ...
        List<PartnerPoliOrgHistory29Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory29Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory29ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory29ItemWriter.write(items);
