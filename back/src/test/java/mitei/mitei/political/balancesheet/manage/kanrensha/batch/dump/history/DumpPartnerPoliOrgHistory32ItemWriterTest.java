...
/**
 * DumpPartnerPoliOrgHistory32ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory32ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org32.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory32ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory32Entity entity00 = new PartnerPoliOrgHistory32Entity();
        ...
        List<PartnerPoliOrgHistory32Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory32Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory32ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory32ItemWriter.write(items);
