...
/**
 * DumpPartnerPoliOrgHistory17ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory17ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org17.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory17ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory17Entity entity00 = new PartnerPoliOrgHistory17Entity();
        ...
        List<PartnerPoliOrgHistory17Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory17Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory17ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory17ItemWriter.write(items);
