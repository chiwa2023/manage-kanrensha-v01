...
/**
 * PartnerCorpAddMiniWkTblFixItemWriter単体テスト
 */
@SpringJUnitConfig
@AutoConfigureMockMvc
@SpringBootTest(webEnvironment = WebEnvironment.RANDOM_PORT)
@DirtiesContext(classMode = ClassMode.BEFORE_CLASS)
class PartnerCorpAddMiniWkTblFixItemWriterTest {
    ...
    void test() throws Exception {
        ...
        final int executeId = 107;
        WkTblPartnerCorpAddMinEntity entityCall = wkTblPartnerCorpAddMinRepository.findById(executeId).get();
        entityCall.setIsFinish(true);
        entityCall.setJudgeReason("正常終了");

        List<WkTblPartnerCorpAddMinEntity> list = new ArrayList<>();
        list.add(entityCall);

        // Chunkを作成してセット
        Chunk<? extends WkTblPartnerCorpAddMinEntity> items = new Chunk<>(list);

        partnerCorpAddMiniWkTblFixItemWriter.beforeStep(this.getStepExecution());
        partnerCorpAddMiniWkTblFixItemWriter.write(items);

        WkTblPartnerCorpAddMinEntity entityAns = wkTblPartnerCorpAddMinRepository.findById(executeId).get();
