...
/**
 * DumpPartnerPoliOrgHistory40ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPoliOrgHistory40ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/poli__org40.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPoliOrgHistory40ItemWriter.beforeStep(stepExecution);

        PartnerPoliOrgHistory40Entity entity00 = new PartnerPoliOrgHistory40Entity();
        ...
        List<PartnerPoliOrgHistory40Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPoliOrgHistory40Entity> items = new Chunk<>(list);
        dumpPartnerPoliOrgHistory40ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPoliOrgHistory40ItemWriter.write(items);
