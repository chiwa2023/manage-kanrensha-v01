...
/**
 * DumpPartnerCorpHistory12ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory12ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_12.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory12ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory12Entity entity00 = new PartnerCorpHistory12Entity();
        ...
        List<PartnerCorpHistory12Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory12Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory12ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory12ItemWriter.write(items);
