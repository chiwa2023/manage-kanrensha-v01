...
/**
 * DumpPartnerPersonHistory47ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory47ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_47.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory47ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory47Entity entity00 = new PartnerPersonHistory47Entity();
        ...
        List<PartnerPersonHistory47Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory47Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory47ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory47ItemWriter.write(items);
