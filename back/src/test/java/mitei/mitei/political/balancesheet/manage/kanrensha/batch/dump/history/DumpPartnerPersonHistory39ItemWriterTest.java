...
/**
 * DumpPartnerPersonHistory39ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerPersonHistory39ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/person_39.csv";
        Path path = Paths.get(storageFolder, output);
        StepExecution stepExecution = this.getStepExecution(path.toString());
        dumpPartnerPersonHistory39ItemWriter.beforeStep(stepExecution);

        PartnerPersonHistory39Entity entity00 = new PartnerPersonHistory39Entity();
        ...
        List<PartnerPersonHistory39Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerPersonHistory39Entity> items = new Chunk<>(list);
        dumpPartnerPersonHistory39ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerPersonHistory39ItemWriter.write(items);
