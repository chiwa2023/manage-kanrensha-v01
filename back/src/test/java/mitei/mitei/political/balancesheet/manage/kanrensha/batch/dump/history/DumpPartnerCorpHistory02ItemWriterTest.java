...
/**
 * DumpPartnerCorpHistory02ItemWriter単体テスト
 */
@SpringJUnitConfig
...
class DumpPartnerCorpHistory02ItemWriterTest {
    // CHECKSTYLE:OFF
    ...
    void test() throws Exception {
        ...
        String output = "/output_test/abc_02.csv";
        Path path = Paths.get(storageFolder, output);

        StepExecution stepExecution = this.getStepExecution(path.toString());

        dumpPartnerCorpHistory02ItemWriter.beforeStep(stepExecution);

        PartnerCorpHistory02Entity entity00 = new PartnerCorpHistory02Entity();
        ...
        List<PartnerCorpHistory02Entity> list = new ArrayList<>();
        list.add(entity00);

        // Chunkを作成してセット
        Chunk<? extends PartnerCorpHistory02Entity> items = new Chunk<>(list);
        dumpPartnerCorpHistory02ItemWriter.open(stepExecution.getExecutionContext());
        dumpPartnerCorpHistory02ItemWriter.write(items);
